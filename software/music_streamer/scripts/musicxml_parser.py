...
#divisions = int(parts.getElementsByTagName('measure')[0].getElementsByTagName('attributes')[0].getElementsByTagName('divisions')[0].nodeValue)
...
# Adjust note duration to be a division of the smallest duration
#min_duration = min(note_list, key = lambda t: t[1])
#print(min_duration)
#note_list = [(note[0], note[1] / int(min_duration)) for note in note_list]
#print(note_list)
...
with open(memory_contents_filepath, 'w') as memory_file:
    for note in note_list:
        if (note[0] == 0):
            ...
            continue
        for i in range(0, 4 * note[1]):
            memory_file.write(str(int(cpu_clock_freq/note[0]/2)) + "\n")
